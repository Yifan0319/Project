...
for ite in range(int(maxiter)):
    if ite % update_interval == 0:
        q, reconst = model.predict(data, verbose=1)  # Calculate soft assignment distribtuion & CAE reconstructions
    ...
    loss = model.train_on_batch(x=data[idx], y=[p[idx], data[idx, :120, :40, :]])
